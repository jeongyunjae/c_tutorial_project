...
## 변수
* 메모리에서 바뀔 수 있는 어떤 값을 보관하는 곳입니다.
* 영어로 Variable (바뀔수 있는 것들) 이라는 뜻입니다.
...
* 접미사가 붙지 않은 정수 상수의 형은 int, long long, unsigned long long 중 하나입니다.
	- 시스템은 세 가지 형 중 그 정수 상수를 표현할 수 있는 첫번째 것을 선택하여 그 정수 상수의 형으로 합니다.
...
## 실수 
* 부동소수로 표현합니다.
* 모든 수를 지수 형태로 변환하여 부호, 지수, 가수부로 나누어 저장합니다.
- 4.0  ->  1.0 x 2^2
	 - 부호 : +
	 - 지수 : 2
	 - 가수 : 1.0
...
* 부동형으로 모든 실수를 표현할 수 없습니다.

* 부동형에 배정될 수 있는 값은 정밀도와 범위라는 속성으로 기술됩니다.
...
## 논리 값과 _Bool 형
* 논리 값을 다루기 위해 _Bool 형이 C99에 추가됐습니다.
* 참과 거짓을 표현하기 위해 사용합니다.
- 거짓 : 0
- 참 : 1
...
---
## 오버플로
- 값의 범위를 초과할 때 발생합니다. (주의 필요)

- 정수 오버플로가 발생해도 프로그램은 계속 수행되지만, 논리적으로 부정확한 값이 계산됩니다.
...
## 자료형 변환
* 산술 수식은 값과 형을 가집니다.
* 산술 수식의 피연산자들의 형이 다르면 피연산자의 형을 변환하여 일치시킵니다.
* 일반적 산술 변환은 컴파일러에 의해 자동적으로 일어납니다.
- 컴파일러는 선언문을 참조하여 이러한 일을 수행합니다.
	- 자동 형 변환(Automatic type conversion)
	- 묵시적 형 변환(Implicit type conversion)

* 정수 승격
	- 정수 수식에서 일반적 산술 변환의 한 단계로 일어납니다.
